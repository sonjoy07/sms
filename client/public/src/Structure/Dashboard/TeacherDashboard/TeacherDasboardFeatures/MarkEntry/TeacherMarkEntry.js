...
const ExamMarksEntry = () => {
    ...
    //get student
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_NODE_API}/api/exam_mark/search?session_id=${session_id}&exam_info_id=${exam_id}&class_id=${class_input}&section_id=${section}&subject_id=${subject_id}&school_info_id=${school_id}`,
            {
                headers: {
                    authorization: "bearer " + localStorage.getItem("access_token"),
                },
            ...
            .then((response) => {
                ...
                response.data.map((stu) => {
                    // console.log(stu)
                    student_mark.push({ student_id: stu.id, student_name: stu.name, student_code: stu.student_code, mark_obtained: '0' });
                });
