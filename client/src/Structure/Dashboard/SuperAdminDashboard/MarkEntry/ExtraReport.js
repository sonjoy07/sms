...
const ExtraReport = () => {
  ...
  const getHomework = () => {
    axios
      .get(
        `${process.env.REACT_APP_NODE_API}/api/activities/all/filter?school_info_id=${school_info_id}&class_id=${class_id}&section_id=${section_id}&subject_id=${subject_id}&&start_date=${startDate}&&end_date=${endDate}&&school_teacher_id=${localStorage.getItem('user_code')}`,
        {
          headers: {
            authorization: "bearer " + localStorage.getItem("access_token"),
          },
