...
const ExamMarksEntry = () => {
  ...
  let handleSearch = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_NODE_API}/api/marks_exist?section_id=${section_id}&&class_id=${class_id}&&session_id=${session_id}&&teacher_id=${user_code}&&exam_id=${exam_id}&&subject_id=${subject_id}`,
        {
          headers: { authorization: "bearer " + access_token },
        }
      )
  ...
  const updateMarks = (event) => {
    if (event.key === 'Enter') {
      fetch(`${process.env.REACT_APP_NODE_API}/api/exam_mark/update`, {
        ...
        .then((json) => {
          toast(`student's Mark updated successful!!`)
          ...
          if (value == '') {
            ...
            for (let k = 0; k < latest_mark.length; k++) {
              if (latest_mark[k].student_id == i.student_id) {
                ...
                latest_mark[k].mark_id = mark_id;
              }
            }
          }
          else {
            latest_mark.push({ student_id: i.student_id, mark_obtained: value, mark_id: mark_id })
          }
        }

      }
    })
    //    let newMarks =  present_mark.filter(info=>{

    //         let exist = markExists.find(res=>res.student_id === info.student_id)
    //         if(exist === undefined){
    //             return info.mark_obtained = 0
    //         }else{
    //             return info.mark_obtained = exist.marks_obtained
    //         }
    //     })
  ...
  const handleMarks = () => {
    ...
    const results = present_mark.filter(res => res.mark_obtained !== '');
