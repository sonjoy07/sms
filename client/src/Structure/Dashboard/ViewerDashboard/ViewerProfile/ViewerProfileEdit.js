...
const ViewerProfileEdit = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setMobileNo(studentData.mobile_number)
        setEmail(studentData.email_address)      
  ...
  const handleSubmit = () => {
    ...
    fetch(`${process.env.REACT_APP_NODE_API}/api/administrator/profile_update?student_code=${student.admin_code}`, {
      method: "POST",
      headers: {
        authorization: "bearer " + localStorage.getItem("access_token"),
      },
...
export default ViewerProfileEdit